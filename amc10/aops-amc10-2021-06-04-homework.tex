...
\end{align*}
If we rearrange and simplify, we get a contradiction:
\begin{align*}
x - 6 & = x - 2 \\ 
    6 & = 2
\end{align*}
Is that it? No! We ought to have noticed that $x=1$ is a solution of the above. There was no need to waste time with rearranging and simplifying. So we can rule out $k=1$. There are four more cases to consider.
...
The other cases could be resolved with similar steps. But trying five cases will take a lot of work. So is there a quicker method? What could be the cause of non-existence? Well if the system above yielded either $x=2$ or $x=6$, we would have a contradiction because the the denominator is not allowed to be zero. So how could that happen? Solve for $k$ and try $x=2$ and $x=6$:
...
\frac{x-1}{x-2} & = \frac{x-k}{x-6} \\
   ...
   x^2 - 7x + 6 & = x^2 - (2+k)x + 2k \\ 
         (5-k)x & = 6-2k
\end{align*}
Oh we can stop here. Since $k=5$ annuls the lhs, we have a contradiction:
\begin{align*}
         0 = -4
\end{align*}
...
\begin{align*}
                  (a-b)^2 & = a^2 + b^2 -2ab \\[1em]
       \frac{(a-b)^2}{ab} & = \frac{a^2 + b^2 -2ab}{ab} \\[1em]
        \frac{(ab)^2}{ab} & = \frac{a^2 + b^2}{ab} - \frac{2ab}{ab} \\[1em]
                       ab & = \frac{a^2 + b^2}{ab} - 2 \\[1em]
\frac{a^2 + b^2}{ab} - ab & =  2 
\end{align*}
And thus
\begin{empheq}[box={\mathbox[colback=white]}]{equation*}
